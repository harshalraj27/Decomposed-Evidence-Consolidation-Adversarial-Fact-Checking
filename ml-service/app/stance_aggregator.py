...
def aggregate_evidences(stance_results):
    ...
    for e in stance_results:
        ...
        # unified ranking / weighting signal
        rank_score = e.get('rerank_score', e['faiss_score'])
        weight = max(0.0, rank_score)
        ...
        if abs(stance_score) < epsilon:
            ...
            supporting.append(e)
            support_strength += max(0.0, evidence_contribution)

        else:  # stance_score < 0
            contradicting.append(e)
            contradict_strength += max(0.0, -evidence_contribution)

    supporting = sorted(
        supporting,
        key=lambda e: e['evidence_contribution'],
        reverse=True
    )
    ...
    neutral = sorted(
        neutral,
        key=lambda e: e.get('rerank_score', e['faiss_score']),
        reverse=True
    )
